...
class InputExample(object):
    ...
    def __init__(self, guid, text, label=None, gazetteer=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, gazetteer, length):
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self, data_dir):
        ...
        with open(tagset_file, encoding='utf-8') as input_stream:
            tagset = input_stream.read().split('\n')
        return tagset + ["X", "[CLS]", "[SEP]"]
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            gazetteer = tokenization.convert_to_unicode(line[2])
            examples.append(InputExample(guid=guid, text=text, label=label, gazetteer=gazetteer))
        return examples
...
def filed_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["gazetteers"] = create_int_feature(feature.gazetteer)
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    name_to_features = {
        ...
        "gazetteers": tf.FixedLenFeature([seq_length], tf.int64),
...
def crf_tagger(logits, tag_ids, lengths, tagset_size=7):
    lengths = tf.squeeze(lengths, axis=-1)
    if tag_ids is not None:
        log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(logits, tag_ids, lengths)
    else:
        log_likelihood, transition_params = tf.truncated_normal(shape=[1]), tf.truncated_normal(
            shape=[tagset_size, tagset_size])
    decoded_tags, best_score = tf.contrib.crf.crf_decode(logits, transition_params, lengths)
    loss = tf.reduce_sum(-log_likelihood)
    return loss, decoded_tags


def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, labels, num_labels, use_one_hot_embeddings, lengths, gazetteers):
    ...
    if FLAGS.use_gazetteers:
        gazetteers_embeddings = tf.get_variable(
            "gazetteer_embeddings", [num_labels, 3], initializer=tf.truncated_normal_initializer(stddev=0.02)
        )
        gazetteers_embedded = tf.nn.embedding_lookup(gazetteers_embeddings, gazetteers)
        output_layer = tf.concat([output_layer, gazetteers_embedded], axis=-1)
    ...
    with tf.variable_scope("loss"):
        ...
        if FLAGS.crf:
            loss, predict = crf_tagger(logits, labels, lengths)
        else:
...
def serving_input_receiver_fn():
    feature_spec = {
        ...
        "gazetteers": tf.FixedLenFeature([FLAGS.max_seq_length], tf.int64),
        "length": tf.FixedLenFeature([1], tf.int64)
    }
    ...
    if FLAGS.do_predict:
        ...
        with open(output_predict_file, 'w') as p_writer:
            for tokens, prediction in zip(tokens_iterator, result):
                slen = len(tokens)
                output_line = "\n".join(
                    id2label[id] if id != 0 else id2label[3] for id in prediction['prediction'][:slen]) + "\n"
                p_writer.write(output_line)
    if FLAGS.export_model:
        estimator._export_to_tpu = False
        estimator.export_saved_model(
            export_dir_base=FLAGS.save_exported_model_to,
